...
const string strMessageMagic = "MNR Signed Message:\n";
...
{
    int64 nSubsidy = 70000 * COIN;


    if(nHeight < 5) // no block reward within the first time
        nSubsidy = 0;
    if(nHeight > 16500000) // no block reward after 5 years
        nSubsidy = 0;

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // MNR: 1 days
static const int64 nTargetSpacing = 10; // MNR: 10 seconds
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal MNR miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "MNR", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
